...
#### Option 1: Local Development with Database (Recommended)
This runs a complete local environment with MongoDB in Docker.
...
- Starts MongoDB in Docker container (port 27017)
- Starts backend API with auto-restart (port 3001)
- Starts frontend dev server with hot-reload (port 3000)
- Connects to local database: `mongodb://localhost:27017/pstudios-dev`

**First-time setup:**
After servers start, create an admin user:
```bash
cd backend
...
- MongoDB (via Mongo Express): http://localhost:8081

#### Option 2: Mock Mode (No Database Required)
Fast startup for UI-only development with mock data. No Docker needed.
...
**Mock Mode Credentials:** admin@pstudios.com / admin123
...
- ✅ **Discord Announcements Module**: Right-sidebar displaying latest announcements from Discord bot
- ✅ **Header Layout Redesign**: Logo positioned at left edge, navigation links centered on viewport
- ✅ **Responsive Logo System**: Automatically switches between vertical and square logos based on screen size
- ✅ **Contact Page Mobile Fix**: Fixed horizontal overflow issue on mobile devices
- ✅ **Deployment Analysis**: Comprehensive readiness checklist created

See `pstudios-landingpage/UI_UPDATES_CHANGELOG.md` for detailed changelog.
